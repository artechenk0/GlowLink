...
function refreshState(state) {
  ...
  if (!state.device) { $("#device-label").textContent = "устройство не выбрано"; $("#link-label").textContent = "Нет устройства"; dot.className = "dot offline"; $("#connection-toggle").textContent = "Подключить"; return; }
  $("#device-label").textContent = `${state.device.name || "без имени"} · ${state.device.address}`;
  $("#link-label").textContent = state.connected ? "Контроллер подключён" : "Контроллер не подключён";
...
function renderDevices(hits) {
  ...
  hits.forEach((hit) => {
    ...
    card.innerHTML = `<span class="device-mark">${BLUETOOTH_ICON}</span><span><strong>${escapeHtml(hit.name || "Устройство без имени")}</strong><small>${escapeHtml(hit.address)}</small></span><span class="signal">${hit.rssi ?? ""} dBm</span>`;
...
window.__led = {
  onScan(result) { $("#scan").disabled = false; $("#scan").textContent = "Найти устройства"; renderDevices(result.hits || []); const state = $("#scan-state"); state.innerHTML = result.kind === "err" ? `<div class="status">${escapeHtml(result.text)}</div>` : `<div class="status good">✓ ${escapeHtml(result.text || "Выберите найденное устройство")}</div>`; $("#scan-again").hidden = false; },
  ...
  onMsg(message) {
    ...
    if (powerBusy && message.text?.startsWith("RGB-подсветка выключена")) { setPowerState(false); setPowerBusy(false); }
...
window.addEventListener("pywebviewready", async () => {
  ...
  $("#power").onclick = () => { if (powerBusy || booting || !window.__ledConnected) return; setPowerBusy(true); $("#power span").textContent = "Выключение…"; Promise.resolve(api().power_off()).catch(() => { setPowerState(powerOn); setPowerBusy(false); status($("#color-status"), "Не удалось отправить команду питания.", "err"); }); };
